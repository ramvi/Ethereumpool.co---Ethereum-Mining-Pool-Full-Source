...
while (1) {
    ...
    if ($state_work == 0) {
        ...
        if (!$coinbase) {
            ...
            $pool_fee_revenue = '0x363e329fbc3d75e5d6e522ed491b1f7b4aff63dc';  //Pool fee revenue
            $poolFee = 100 - 5.86;  // fee -> 1%
            ...
            while ($row = mysqli_fetch_row($existResultMinersss)) {
                ...
                if ($minedBy == $coinbase) {
                    ...
                    $output = 0;
                    $output = shell_exec('/usr/bin/python /home/ramvi/workspace/pyethereum/ethereum/nonce_fast.py ' . $filtempe . ' ' . rand(5, 123142312));
                    $test = explode(' ', $output);
